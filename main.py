...
def developer(desarollador):
  ...
  total_datos = df[(df['developer']=='CAPCOM Co., Ltd.')].groupby(["year"]).count()["developer"]
  datos_free = df[(df['developer']=='CAPCOM Co., Ltd.')&(df['price']=="Free")].groupby(["year"]).count()["developer"]
